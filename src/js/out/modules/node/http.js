(function () {"use strict";
  var $;
  const EventEmitter = @requireId(13), { isTypedArray } = @requireBuiltin(1030), headerCharRegex = /[^\t\x20-\x7e\x80-\xff]/;
  ...
  const { URL } = globalThis, globalReportError = globalThis.reportError, setTimeout = globalThis.setTimeout, fetch = Bun.fetch, nop = () => {
  ...
  var _defaultHTTPSAgent, kInternalRequest = Symbol("kInternalRequest"), kInternalSocketData = Symbol.for("::bunternal::");
  ...
  class IncomingMessage extends Readable {
    method;
    complete;
    constructor(req, defaultIncomingOpts2) {
      const method = req.method;
      super();
      const url = new URL(req.url);
      var { type = "request", [kInternalRequest]: nodeReq } = defaultIncomingOpts2 || {};
  ...
  function emitErrorNt(msg, err, callback) {
    if (callback(err), typeof msg.emit === "function" && !msg._closed)
      msg.emit("error", err);
  }
  ...
  function write_(msg, chunk, encoding, callback, fromEnd) {
    ...
    if (!msg._hasBody)
      return debug("This type of response MUST NOT have a body. Ignoring write() calls."), process.nextTick(callback), !0;
    return !0;
  }
  ...
  OriginalWriteHeadFn = ServerResponse.prototype.writeHead, OriginalImplicitHeadFn = ServerResponse.prototype._implicitHeader;
