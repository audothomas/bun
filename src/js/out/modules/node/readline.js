(function () {"use strict";
  var $;
  const EventEmitter = @requireId(13), { StringDecoder } = @requireBuiltin(1027);
  var isWritable, { inspect } = Bun, debug = process.env.BUN_JS_DEBUG ? console.log : () => {
    ...
    return width;
  }, ansiPattern = "[\\u001B\\u009B][[\\]()#;?]*(?:(?:(?:(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]+)*|[a-zA-Z\\d]+(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]*)*)?\\u0007)|(?:(?:\\d{1,4}(?:;\\d{0,4})*)?[\\dA-PR-TZcf-ntqry=><~]))", ansi = new RegExp(ansiPattern, "g");
  function stripVTControlCharacters(str) {
    return validateString(str, "str"), RegExpPrototypeSymbolReplace.call(ansi, str, "");
  }
  var kCustomPromisifiedSymbol = SymbolFor("nodejs.util.promisify.custom"), kCustomPromisifyArgsSymbol = Symbol("customPromisifyArgs");
  ...
  function getNodeErrorByName(typeName) {
    ...
    return errorBases[typeName] = NodeError2, NodeError2;
  }
  var NodeError = getNodeErrorByName("Error"), NodeTypeError = getNodeErrorByName("TypeError"), NodeRangeError = getNodeErrorByName("RangeError");
  ...
  function validateObject(value, name, options = null) {
    var allowArray = options?.allowArray ?? !1, allowFunction = options?.allowFunction ?? !1, nullable = options?.nullable ?? !1;
  ...
  function validateUint32(value, name, positive = !1) {
    if (typeof value !== "number")
      throw new ERR_INVALID_ARG_TYPE(name, "number", value);
    if (!NumberIsInteger(value))
      throw new ERR_OUT_OF_RANGE(name, "an integer", value);
  ...
  CSI.kEscape = kEscape, CSI.kClearLine = kClearLine = CSI`2K`, CSI.kClearScreenDown = kClearScreenDown = CSI`0J`, CSI.kClearToLineBeginning = kClearToLineBeginning = CSI`1K`, CSI.kClearToLineEnd = kClearToLineEnd = CSI`0K`;
  ...
  function emitKeypressEvents(stream, iface = {}) {
    ...
    function onData2(input) {
      if (stream.listenerCount("keypress") > 0) {
        ...
        stream.removeListener("data", onData2), stream.on("newListener", onNewListener);
    }
    function onNewListener(event) {
      if (event === "keypress")
        stream.on("data", onData2), stream.removeListener("newListener", onNewListener);
    }
    if (stream.listenerCount("keypress") > 0)
  ...
  ObjectSetPrototypeOf(Interface.prototype, _Interface.prototype), ObjectSetPrototypeOf(Interface, _Interface), Interface.prototype.question = function question(query, options, cb) {
  ...
  }), Interface.prototype._setRawMode = _Interface.prototype[kSetRawMode], Interface.prototype._onLine = _Interface.prototype[kOnLine], Interface.prototype._writeToOutput = _Interface.prototype[kWriteToOutput], Interface.prototype._addHistory = _Interface.prototype[kAddHistory], Interface.prototype._refreshLine = _Interface.prototype[kRefreshLine], Interface.prototype._normalWrite = _Interface.prototype[kNormalWrite], Interface.prototype._insertString = _Interface.prototype[kInsertString], Interface.prototype._tabComplete = function(lastKeypressWasTab) {
    this.pause();
  ...
  class Readline {
    ...
    constructor(stream, options = void 0) {
      if (isWritable ??= @requireId(30).isWritable, !isWritable(stream))
  ...
  var PromisesInterface = class Interface2 extends _Interface {
    constructor(input, output, completer, terminal) {
      super(input, output, completer, terminal);
    }
