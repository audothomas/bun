(function () {"use strict";
  const EventEmitter = @requireId(13), StreamModule = @requireId(30), {
    constants: { signals }
  } = @requireId(21), { promisify } = @requireId(38);
  var { create: ObjectCreate, assign: ObjectAssign, defineProperty: ObjectDefineProperty } = Object, BufferConcat = Buffer.concat, BufferIsEncoding = Buffer.isEncoding, kEmptyObject = ObjectCreate(null), ArrayPrototypePush = Array.prototype.push, ArrayPrototypeJoin = Array.prototype.join, ArrayPrototypeMap = Array.prototype.map, ArrayPrototypeIncludes = Array.prototype.includes, ArrayPrototypeSlice = Array.prototype.slice, ArrayPrototypeUnshift = Array.prototype.unshift, ArrayIsArray = Array.isArray, ArrayBufferIsView = ArrayBuffer.isView, NumberIsInteger = Number.isInteger, MathAbs = Math.abs, StringPrototypeToUpperCase = String.prototype.toUpperCase, StringPrototypeIncludes = String.prototype.includes, StringPrototypeSlice = String.prototype.slice, Uint8ArrayPrototypeIncludes = Uint8Array.prototype.includes;
  const MAX_BUFFER = 1048576, __DEBUG__ = process.env.DEBUG || !1, __TRACK_STDIO__ = process.env.DEBUG_STDIO, debug = __DEBUG__ ? console.log : () => {
  };
  if (__TRACK_STDIO__)
  ...
  var NativeWritable, ReadableFromWeb;
  ...
  function sanitizeKillSignal(killSignal2) {
  ...
  function abortChildProcess(child, killSignal2) {
    if (!child)
      return;
    try {
  ...
  }, validateOneOf = (value, name, oneOf) => {
    if (!ArrayPrototypeIncludes.call(oneOf, value)) {
      const reason = "must be one of: " + ArrayPrototypeJoin.call(ArrayPrototypeMap.call(oneOf, (v) => typeof v === "string" ? `'${v}'` : String(v)), ", ");
      throw new ERR_INVALID_ARG_VALUE(name, value, reason);
    }
  }, validateObject = (value, name, options = null) => {
  ...
  var { Error, TypeError, RangeError } = globalThis;
  ...
  function ERR_OUT_OF_RANGE(str, range, input, replaceDefaultBoolean = !1) {
