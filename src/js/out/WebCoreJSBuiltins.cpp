...
const int s_processObjectInternalsGetStdioWriteStreamCodeLength = 4533;
static const JSC::Intrinsic s_processObjectInternalsGetStdioWriteStreamCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_processObjectInternalsGetStdioWriteStreamCode = "(function (X,Z){\"use strict\";var N=(K)=>{var j=@requireMap.get(K);if(j)return j.exports;return @internalRequire(K)},I={path:\"node:process\",require:N};function q(K){var{Duplex:j,eos:B,destroy:G}=N(\"node:stream\"),L=class Q extends j{#B;#$;#N=!0;#O=!0;#K;#G;#j;#H;#J;#L;get isTTY(){return this.#L\?\?=N(\"node:tty\").isatty(K)}get fd(){return K}constructor(H){super({readable:!0,writable:!0});this.#K=`/dev/fd/${H}`}#M(H){const J=this.#G;if(this.#G=null,J)J(H);else if(H)this.destroy(H);else if(!this.#N&&!this.#O)this.destroy()}_destroy(H,J){if(!H&&this.#G!==null){var O=class D extends Error{code;name;constructor(V=\"The operation was aborted\",M=void 0){if(M!==void 0&&typeof M!==\"object\")throw new Error(`Invalid AbortError options:\\n\\n${JSON.stringify(M,null,2)}`);super(V,M);this.code=\"ABORT_ERR\",this.name=\"AbortError\"}};H=new O}if(this.#j=null,this.#H=null,this.#G===null)J(H);else{if(this.#G=J,this.#B)G(this.#B,H);if(this.#$)G(this.#$,H)}}_write(H,J,O){if(!this.#B){var{createWriteStream:D}=N(\"node:fs\"),V=this.#B=D(this.#K);V.on(\"finish\",()=>{if(this.#H){const M=this.#H;this.#H=null,M()}}),V.on(\"drain\",()=>{if(this.#j){const M=this.#j;this.#j=null,M()}}),B(V,(M)=>{if(this.#O=!1,M)G(V,M);this.#M(M)})}if(V.write(H,J))O();else this.#j=O}_final(H){this.#B&&this.#B.end(),this.#H=H}#Q(){var{createReadStream:H}=N(\"node:fs\"),J=this.#$=H(this.#K);return J.on(\"readable\",()=>{if(this.#J){const O=this.#J;this.#J=null,O()}else this.read()}),J.on(\"end\",()=>{this.push(null)}),B(J,(O)=>{if(this.#N=!1,O)G(J,O);this.#M(O)}),J}_read(){var H=this.#$;if(!H)H=this.#Q();while(!0){const J=H.read();if(J===null||!this.push(J))return}}};return new L(K)}var x=N(\"node:events\");function P(K){if(!K)return!0;var j=K.toLowerCase();return j===\"utf8\"||j===\"utf-8\"||j===\"buffer\"||j===\"binary\"}var Y,U=[0,0];console.log(\"b\",x);var T=class K extends x{#B;#$;#N;#O;bytesWritten=0;setDefaultEncoding(j){if(this.#$||!P(j))return this.#j(),this.#$.setDefaultEncoding(j)}#K(){switch(this.#B){case 1:{var j=@Bun.stdout.writer({highWaterMark:0});return j.unref(),j}case 2:{var j=@Bun.stderr.writer({highWaterMark:0});return j.unref(),j}default:throw new Error(\"Unsupported writer\")}}#G(){return this.#N\?\?=this.#K()}constructor(j){super();this.#B=j}get fd(){return this.#B}ref(){this.#G().ref()}unref(){this.#G().unref()}on(j,B){if(j===\"close\"||j===\"finish\")return this.#j(),this.#$.on(j,B);if(j===\"drain\")return super.on(\"drain\",B);if(j===\"error\")return super.on(\"error\",B);return super.on(j,B)}get _writableState(){return this.#j(),this.#$._writableState}get _readableState(){return this.#j(),this.#$._readableState}pipe(j){return this.#j(),this.#$.pipe(j)}unpipe(j){return this.#j(),this.#$.unpipe(j)}#j(){if(this.#$)return;this.#$=q(this.#B);const j=this.eventNames();for(let B of j)this.#$.on(B,(...G)=>{this.emit(B,...G)})}#H(j){var B=this.#G();const G=B.write(j);this.bytesWritten+=G;const L=B.flush(!1);return!!(G||L)}#J(j,B){if(!P(B))return this.#j(),this.#$.write(j,B);return this.#H(j)}#L(j,B){if(B)this.emit(\"error\",B);try{j(B\?B:null)}catch(G){this.emit(\"error\",G)}}#M(j,B,G){if(!P(B))return this.#j(),this.#$.write(j,B,G);var L=this.#G();const Q=L.write(j),H=L.flush(!0);if(H\?.then)return H.then(()=>{this.#L(G),this.emit(\"drain\")},(J)=>this.#L(G,J)),!1;return queueMicrotask(()=>{this.#L(G)}),!!(Q||H)}get isTTY(){return!1}write(j,B,G){const L=this._write(j,B,G);if(L)this.emit(\"drain\");return L}get hasColors(){return @Bun.tty[this.#B].hasColors}_write(j,B,G){var L=this.#$;if(L)return L.write(j,B,G);switch(arguments.length){case 0:{var Q=new Error(\"Invalid arguments\");throw Q.code=\"ERR_INVALID_ARG_TYPE\",Q}case 1:return this.#H(j);case 2:if(typeof B===\"function\")return this.#M(j,\"\",B);else if(typeof B===\"string\")return this.#J(j,B);default:{if(typeof B!==\"undefined\"&&typeof B!==\"string\"||typeof G!==\"undefined\"&&typeof G!==\"function\"){var Q=new Error(\"Invalid arguments\");throw Q.code=\"ERR_INVALID_ARG_TYPE\",Q}if(typeof G===\"undefined\")return this.#J(j,B);return this.#M(j,B,G)}}}destroy(){return this}end(){return this}};if(console.log(\"a\",T),Z(X,U)){var C=class K extends T{get isTTY(){return!0}cursorTo(j,B,G){return(Y\?\?=N(\"node:readline\")).cursorTo(this,j,B,G)}moveCursor(j,B,G){return(Y\?\?=N(\"node:readline\")).moveCursor(this,j,B,G)}clearLine(j,B){return(Y\?\?=N(\"node:readline\")).clearLine(this,j,B)}clearScreenDown(j){return(Y\?\?=N(\"node:readline\")).clearScreenDown(this,j)}getWindowSize(){if(Z(X,U)===!0)return[U[0],U[1]]}get columns(){if(Z(X,U)===!0)return U[0]}get rows(){if(Z(X,U)===!0)return U[1]}};return new C(X)}return new T(X)})\n";
...
const int s_processObjectInternalsGetStdinStreamCodeLength = 1874;
static const JSC::Intrinsic s_processObjectInternalsGetStdinStreamCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_processObjectInternalsGetStdinStreamCode = "(function (K){\"use strict\";var I=(B)=>{var J=@requireMap.get(B);if(J)return J.exports;return console.log(\"calling $internalRequire\",B),@internalRequire(B)},{Duplex:N,eos:P,destroy:L}=I(\"node:stream\"),Q=class B extends N{#I;#M;#$;#B=!0;#G=!1;#K=!0;#j;#J;#z;get isTTY(){return I(\"node:tty\").isatty(K)}get fd(){return K}constructor(){super({readable:!0,writable:!0})}#H(J){const M=this.#J;if(this.#J=null,M)M(J);else if(J)this.destroy(J);else if(!this.#B&&!this.#K)this.destroy()}_destroy(J,M){if(!J&&this.#J!==null){var j=class z extends Error{constructor(G=\"The operation was aborted\",H=void 0){if(H!==void 0&&typeof H!==\"object\")throw new Error(`Invalid AbortError options:\\n\\n${JSON.stringify(H,null,2)}`);super(G,H);this.code=\"ABORT_ERR\",this.name=\"AbortError\"}};J=new j}if(this.#J===null)M(J);else if(this.#J=M,this.#$)L(this.#$,J)}setRawMode(J){}on(J,M){if(J===\"readable\")this.ref(),this.#G=!0;return super.on(J,M)}pause(){return this.unref(),super.pause()}resume(){return this.ref(),super.resume()}ref(){this.#I\?\?=@Bun.stdin.stream().getReader(),this.#M\?\?=setInterval(()=>{},1<<30)}unref(){if(this.#M)clearInterval(this.#M),this.#M=null}async#L(){try{var J,M;const j=this.#I.readMany();if(!j\?.then)({done:J,value:M}=j);else({done:J,value:M}=await j);if(!J){this.push(M[0]);const z=M.length;for(let G=1;G<z;G++)this.push(M[G])}else this.push(null),this.pause(),this.#B=!1,this.#H()}catch(j){this.#B=!1,this.#H(j)}}_read(J){if(this.#G)this.unref(),this.#G=!1;this.#L()}#N(){var{createWriteStream:J}=I(\"node:fs\"),M=this.#$=J(\"/dev/fd/0\");return M.on(\"finish\",()=>{if(this.#j){const j=this.#j;this.#j=null,j()}}),M.on(\"drain\",()=>{if(this.#z){const j=this.#z;this.#z=null,j()}}),P(M,(j)=>{if(this.#K=!1,j)L(M,j);this.#H(j)}),M}_write(J,M,j){var z=this.#$;if(!z)z=this.#N();if(z.write(J,M))j();else this.#z=j}_final(J){this.#$.end(),this.#j=(...M)=>J(...M)}};return new Q})\n";
